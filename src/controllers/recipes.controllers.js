...
const recipeController = {
    ...
    try {
      ...
      recipeModel
        ...
        .then(async (result) => {
          ...
          await cache.invalidate('recipes:')
  ...
  recipeList: async (req, res) => {
    try {
      ...
      const cacheKey = `recipes:list:${field}:${type}:${getPage}:${limitPage}:${name}`
      const cached = await cache.get(cacheKey)
      if (cached) {
        return success(res, cached.rows, 'success', 'get recipe success (cache)', cached.pagination)
      }
      recipeModel
        ...
        .then(async (result) => {
          ...
          await cache.set(cacheKey, { rows: result.rows, pagination }, 60) // cache 60s
          success(
            res,
            result.rows,
            'success',
  ...
  showRecipeById: (req, res) => {
    try {
      ...
      cache.get(cacheKey).then((cached) => {
        if (cached) return success(res, cached, 'success', 'mendapatkan resep berdasarkan id berhasil (cache)')
        recipeModel
          .showRecipeById(id)
          .then(async (result) => {
            if (result.rowCount > 0) {
              await cache.set(cacheKey, result.rows, 300) // cache detail 5min
              success(
                res,
                result.rows,
                'success',
